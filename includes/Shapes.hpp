...
namespace thor::Shapes
{
	...
	/// @param pointPerCorner number of point per corner.
	/// @return A sf::ConvexShape copy containing the rounded rectangle.
	sf::ConvexShape  roundedRect(sf::Vector2f size, float cornerRadius,unsigned pointPerCorner);
	...
	sf::ConvexShape 	star(std::size_t nbStarPoints, float innerRadius, float outerRadius, const sf::Color& fillColor,
									float outlineThickness = 0.f, const sf::Color& outlineColor = sf::Color());
