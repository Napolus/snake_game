...
template <typename R, typename I, class O>
typename ResourceHolder<R, I, O>::Resource ResourceHolder<R, I, O>::get(const I& id)
{
    auto found = mMap.find(id);
    if (found == mMap.end())
...
template <typename R, typename I, class O>
typename ResourceHolder<R, I, O>::ConstResource ResourceHolder<R, I, O>::get(const I& id) const
{
    auto found = mMap.find(id);
    if (found == mMap.end())
        throw ResourceAccessException("Failed to access resource, ID not currently stored in ResourceHolder");
