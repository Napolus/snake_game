...
struct ActionContext
{
    ...
    using EventPtr = std::shared_ptr<sf::Event>;
    using ClockPtr = std::shared_ptr<sf::Clock>;
	// Constructor
	ActionContext(ID::EVENT_ID id,WindowPtr window,EventPtr event=nullptr,const ClockPtr deltatime=nullptr)
	: window(window),
	  event(event),
	  actionId(id),
	  deltatime(deltatime)
    ...
    ID::EVENT_ID					actionId;


	/// @brief delta time since last processEvents of window
    ClockPtr            deltatime;
};
