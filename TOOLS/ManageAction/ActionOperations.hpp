...
namespace thor::detail
{
	...
	struct ActionResult
	{
                                    ActionResult(): eventContainer(), nbRealtimeTriggers(0u){}
    ...
    private:
        bool containsEvent(const EventBuffer& buffer) const;

        bool filterEvents(const EventBuffer& buffer,std::vector<sf::Event> &out) const;
		protected:
			sf::Event					mEvent;
	};
	...
	{
		public:
										OrNode(ActionNode::ActionNodePtr lhs,ActionNode::ActionNodePtr rhs);
			virtual bool				isActionActive(const EventBuffer& buffer) const;
			virtual bool				isActionActive(const EventBuffer& buffer, ActionResult& out) const;

		private:
			ActionNode::ActionNodePtr		mLhs;
			ActionNode::ActionNodePtr		mRhs;
	};
	...
	{
		public:
										AndNode(ActionNode::ActionNodePtr lhs,ActionNode::ActionNodePtr rhs);
			virtual	bool				isActionActive(const EventBuffer& buffer) const;
			virtual bool				isActionActive(const EventBuffer& buffer, ActionResult& out) const;
	...
	{
		public:
			explicit					NotNode(ActionNode::ActionNodePtr action);
			virtual	bool				isActionActive(const EventBuffer& buffer) const;
			virtual bool				isActionActive(const EventBuffer& buffer, ActionResult& out) const;

		private:
			ActionNode::ActionNodePtr		mAction;
	};

} // namespace thor::detail

#include "ActionOperations.inl"
